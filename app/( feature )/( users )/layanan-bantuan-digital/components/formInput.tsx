...
import { Sparkles, ChevronRight, CheckCircle2, Loader2, ArrowLeft } from "lucide-react";
import Link from "next/link";

const MapView = dynamic(() => import("./MapView"), { ssr: false });

const CATEGORIES = [
    { id: "desain", label: "Desain Grafis", icon: "🎨" },
    { id: "video", label: "Edit Video / Foto", icon: "🎬" },
    { id: "sosmed", label: "Kelola Media Sosial", icon: "📱" },
    { id: "konten", label: "Penulisan Konten", icon: "✍️" },
    { id: "web", label: "Website / Landing Page", icon: "💻" },
    { id: "data", label: "Input / Olah Data", icon: "📊" },
    { id: "admin", label: "Bantuan Administrasi", icon: "🗂️" },
...
function processWithAI(
    ...
    return new Promise((resolve) => {
        setTimeout(() => {
            const keywords = [
                { pattern: /logo|banner|poster|flyer|desain/i, task: "Buat desain grafis (logo / banner / poster)" },
                { pattern: /edit.?video|reels|youtube|shorts/i, task: "Edit video konten" },
                { pattern: /edit.?foto|retouching|thumbnail/i, task: "Edit dan retouch foto" },
                { pattern: /sosmed|instagram|tiktok|twitter|posting/i, task: "Kelola dan buat konten media sosial" },
                { pattern: /artikel|blog|caption|copywriting|tulisan/i, task: "Penulisan artikel / caption / copywriting" },
                { pattern: /website|landing.?page|wordpress|html/i, task: "Buat atau update website / landing page" },
                { pattern: /excel|spreadsheet|data|tabel|input/i, task: "Input dan olah data di spreadsheet" },
                { pattern: /presentasi|slide|powerpoint/i, task: "Buat presentasi / slide deck" },
                { pattern: /admin|email|jadwal|dokumen/i, task: "Bantuan administrasi & manajemen dokumen" },
            ];
            ...
            const minPrice = Math.max(found.length * 25000, 50000);
        ...
        <div className="relative w-full h-full bg-gray-100">
            ...
            <div className="absolute top-4 right-4 z-[1000] flex items-center gap-3">
                <div className="bg-background/90 backdrop-blur-sm border border-border/50 rounded-full px-3.5 py-2 shadow-lg flex items-center gap-2">
                    ...
                    <span className="text-[11px] font-semibold text-foreground">Freelancer tersedia</span>
                </div>
            ...
            <div className="absolute top-4 left-4 bottom-4 z-[1000] w-[400px] flex flex-col rounded-3xl bg-background/95 backdrop-blur-xl shadow-2xl shadow-black/20 border border-border/40 overflow-hidden">
                ...
                <div className="flex items-center gap-3 px-5 pt-5 pb-4 border-b border-border/40 shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <h1 className="text-base font-bold text-foreground truncate leading-tight">Bantuan Digital</h1>
                    </div>
                        ...
                        <div className="flex flex-col flex-1">
                            <div className="px-5 py-4 space-y-4 border-b border-border/40">
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-sm font-semibold text-foreground">Jelaskan kebutuhanmu</label>
                                    <textarea
                                        value={rawText}
                                        onChange={(e) => setRawText(e.target.value)}
                                        placeholder="Ceritakan detail pekerjaannya... misal: butuh logo minimalis untuk brand kopi, ukuran 1:1, warna earthy tone"
                                        rows={4}
                        ...
                        <div className="flex flex-col flex-1 items-center justify-center px-5 py-8 text-center space-y-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground mt-1 max-w-[240px] mx-auto leading-relaxed">
                                    Freelancer terdekat akan segera mengambil proyekmu.
                                </p>
                            </div>
                            <div className="bg-muted/40 border border-border/40 rounded-2xl px-4 py-4 w-full space-y-2.5 text-left text-xs">
                                ...
                                {deadline && (
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-semibold text-foreground">{new Date(deadline).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}</span>
                                    </div>
                                )}
                                <div className="flex justify-between">
                ...
                <div className="shrink-0 px-5 py-3 border-t border-border/30 flex items-center justify-between">
                    <span className="text-[10px] text-muted-foreground/60 font-medium">© Bounty · Remote Work</span>
                    <div className="flex items-center gap-1.5">
                        <div className="w-1.5 h-1.5 rounded-full bg-blue-500" />
                        <span className="text-[10px] text-muted-foreground/60">Freelancer online</span>
                    </div>
                </div>
            </div>
        </div>
